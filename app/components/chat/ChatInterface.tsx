...
export default function ChatInterface({ messages, onSendMessage, isLoading, currentProcessStep, onAddToBoard, showAddToBoardButtons }: ChatInterfaceProps) {
    ...
    return (
        <div className="shadow-lg h-full flex flex-col relative" style={{ backgroundColor: '#0f0f10' }}>
            {/* Messages area - grows to accommodate chat */}
            <div className={`flex-1 transition-all duration-500 ease-in-out ${
                messages.length === 0 && !isAnimating 
                    ? 'flex items-center justify-center' 
                    : 'overflow-y-auto pt-4'
            }`}>
                {/* Header text - fades out during animation */}
                {messages.length === 0 && !isAnimating && (
                    <div className="text-center mb-8 px-4 transition-opacity duration-500">
            ...
            {/* Single input form that moves smoothly */}
            <div className={`transition-all duration-700 ease-in-out px-4 ${
                messages.length === 0 && !isAnimating 
